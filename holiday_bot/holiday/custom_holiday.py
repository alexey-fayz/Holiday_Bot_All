...
from holiday_bot.holiday_bot.holiday.custom_exception import InvalidDateError, HolidayLoadError
from holiday_bot.holiday_bot.holiday.config import LoggingConfig, AppConfig, get_default_config
...
class MyHolidays(Russia):
    ...
    def __init__(self, config: AppConfig = None, **kwargs):
        # Если конфиг не передан, используем дефолтный
    ...
    def _load_custom_holidays(self) -> Dict[date, List[str]]:
        customholidays_file = Path(__file__).parent / self.config.db.file_path
        try:
            custom_hols = {}
            with open(customholidays_file, encoding="utf-8") as f:
                for holiday, name in json.load(f).items():
                    if not re.fullmatch(r"\d{4}-\d{2}-\d{2}", holiday):
                        raise InvalidDateError(date=holiday)
                    else:
            ...
            raise HolidayLoadError(customholidays_file)
