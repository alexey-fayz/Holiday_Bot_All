...
## Задание 7.3. Кастомные исключения
### Контекст

На предыдущих шагах мы научились:
обрабатывать ошибки чтения файла с помощью `try / except `
выбрасывать стандартные исключения (`ValueError`) с помощью `raise` 
логировать ошибки, не прерывая работу приложения. 

Однако в реальных проектах часто используются кастомные (пользовательские) исключения, которые позволяют: 
более точно описывать тип ошибки передавать дополнительные данные (например, путь к файлу или некорректную дату) 
писать более понятные и проверяемые тесты.

В нашем проекте телеграм-бота мы хотим явно различать: ошибку загрузки файла с праздниками ошибку некорректного формата даты праздника.

### Цель задания

Научиться создавать и использовать кастомные исключения и выбрасывать разные типы исключений в зависимости от ситуации.

### Что нужно сделать

В проекте нужно реализовать два кастомных исключения в файле `holiday/custom_exception.py`:

* `InvalidDateError`
* `HolidayLoadError`

Реализовать в каждом исключении методы `__init__` и `__str__` с сообщением по умолчанию - 
* `"Ошибка чтения файла, проверьте корректность пути к файлу или его данные - {}"`
* `"Добавлена дата - {} в неправильном формате '%Y-%m-%d'"`

Например:
```python 
def __init__(self, date, message="Добавлена дата - {} в неправильном формате '%Y-%m-%d'"):
```
И для `str` метода нужно возвращать результат, где вместо `{}` путь до файла или дата в неправильном формате.
```python 
def __str__(self):
```
Задача — доработать функцию загрузки `holiday_bot/holiday/custom_holiday.py` в методе `_load_custom_holidays` кастомных праздников так, чтобы она:
* выбрасывала `InvalidDateError`, если дата праздника имеет некорректный формат
* выбрасывала `HolidayLoadError`, если файл с праздниками не может быть прочитан

Вместо `Exception` так как это слишком широкое исключение, нужно использовать - 
```python 
             ...
             raise HolidayLoadError(customholidays_file)
```
А для `ValueError` заменить на `InvalidDateError`

### Работа с git

* Склонируйте репозиторий задания
* Создайте новую ветку: `git checkout -b 20_01_custom_exceptions`
* Внесите изменения в `holiday_bot/custom_holiday.py`
* Закоммитьте изменения
* Отправьте ветку в репозиторий
* После успешного прохождения тестов создайте Merge Request
* При необходимости установите тег `ready_20`

## Задание 8.1. Декоратор log_calls

### Контекст:
В проекте телеграм-бота нам нужно отслеживать, сколько раз вызываются методы бота. Это полезно для отладки, логирования и сбора статистики. Для этого мы используем декоратор `log_calls`, который увеличивает счётчик вызовов и пишет лог при каждом запуске функции.

### Цель задания:
Реализовать декоратор `log_calls`, который:
увеличивает счётчик вызовов функции (`CALL_COUNTER`)
логирует факт запуска функции через `logger.warning`
работает с асинхронными функциями (`async def`) нужно использовать для `wrapper`
при возникновении ошибки логирует её через `logger.exception` и пробрасывает дальше

### Что нужно сделать:

В файле `holiday_bot/holiday_bot/holiday_launch.py` нужно реализовать декоратор `log_calls`.

Реализуйте её так, чтобы она выполняла следующие шаги:
```python 
from functools import wraps
```
Используйте `@wraps(func)`, чтобы сохранить метаданные функции для wrapper в своей реализации.
Внутри wrapper-а увеличивайте счётчик:
```python 
CALL_COUNTER[func.__qualname__] += 1
count = CALL_COUNTER[func.__qualname__]
```
Логируйте запуск функции через logger.warning:
```python 
logger.warning(f"▶ Запуск: {func.__qualname__} | вызов №{count}")
```
Вызовите оригинальную функцию через `await func(*args, **kwargs)` (так как функция асинхронная)
Если возникает исключение, логируйте его через `logger.exception` и пробрасывайте дальше:
```python 
except Exception:
    logger.exception(f"✗ Ошибка в {func.__qualname__}")
    raise
```
Возвращайте wrapper из декоратора.
В качестве примера можно смотреть на `log_exceptions` из [holiday_launch.py]

### Работа с git

* Склонируйте репозиторий задания
* Создайте новую ветку: `git checkout -b 21_01_decorator`
* Внесите изменения
* Закоммитьте изменения
* Отправьте ветку в репозиторий
* После успешного прохождения тестов создайте Merge Request
* При необходимости установите тег `ready_21`

## Задание 8.2. Декоратор с параметрами log_calls(show_args=True)

### Контекст:
Иногда простого логирования факта вызова функции недостаточно. Для более глубокой отладки бывает полезно видеть, с какими именно аргументами был произведен вызов. Однако выводить аргументы всегда — плохая практика, так как это может засорить логи конфиденциальной информацией. Поэтому нам нужен гибкий инструмент: декоратор, который может принимать параметры. 

### Цель задания:
Модернизировать созданный ранее декоратор `log_calls` так, чтобы он мог принимать необязательный параметр show_args. Если параметр установлен в True, декоратор должен дополнительно выводить в консоль значения args и kwargs. 

### Что нужно сделать:
В файле `holiday_bot/holiday_launch.py` измените реализацию декоратора `log_calls`, превратив его в фабрику декораторов.

Добавьте внешний уровень вложенности: Теперь функция `log_calls` должна принимать параметр show_args (по умолчанию False). 

**Создайте функцию-декоратор:** Внутри `log_calls` определите функцию `decorator(func)`, которая будет принимать оригинальную функцию. 

**Реализуйте логику вывода аргументов:**

Внутри wrapper создайте строку `args_str`.

Если `show_args` равен `True`, сформируйте строку в формате: | Args: {args} {kwargs}. 

Если `show_args` равен `False`, строка должна остаться пустой. 

**Обновите логер:**

Измените сообщение в `logger.warning`, добавив в конец созданную строку `args_str`. 

**Сохраните существующий функционал:**

Увеличение счетчика `CALL_COUNTER` должно работать по-прежнему. 

Обработка исключений через `try-except` и `logger.exception` должна остаться неизменной. 

**Пример использования:**
 
```python
@log_calls(show_args=True)
async def add_holiday_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    pass
 ```
 В логе будет:` ▶ Запуск: some_bot_function | вызов №1 | Args: (12345,) {'mode': 'all'}`
Работа с git:
* Склонируйте репозиторий задания. 
* Создайте новую ветку: git checkout -b 22_01_decorator_args
* Внесите изменения в holiday_bot/holiday_launch.py. 
* Закоммитьте изменения с понятным сообщением.
* Отправьте ветку в репозиторий и создайте Merge Request.
* При необходимости установите тег ready_22.

 **Когда вы используете декоратор с параметрами (например, @log_calls(show_args=True)), вы фактически вызываете функцию log_calls, которая возвращает decorator. Но если вы написали просто @log_calls - (без скобок), Python передает саму функцию в log_calls в качестве первого аргумента.**

## Задание 9. Работа с внешними API и библиотекой requests

### Контекст:

Для того чтобы наш бот мог предоставлять актуальную информацию о праздниках разных стран, нам необходимо научиться получать данные из внешних источников в режиме реального времени. В этом задании мы интегрируем работу с публичным API сервиса `Nager.Date`, который предоставляет данные о государственных праздниках по всему миру.

### Цель задания:

Научиться выполнять HTTP-запросы к REST API, обрабатывать полученные ответы в формате JSON и обеспечивать базовую устойчивость программы к сетевым ошибкам.

### Что нужно сделать:

В файле `holiday_bot/holiday/handler.py` необходимо реализовать функцию `fetch_holidays_from_api`, которая будет запрашивать список официальных праздников через интернет.

1. **Реализуйте сигнатуру функции**: функция должна принимать два обязательных аргумента: `year` (инт) и `country_code` (строка, например, 'RU').
2. **Сформируйте целевой URL**: используйте f-строку для подстановки параметров в адрес: `https://date.nager.at/api/v3/publicholidays/{year}/{country_code}`.
3. **Выполните GET-запрос**:
* Используйте библиотеку `requests` для отправки запроса.

**Важно**: установите параметр `timeout=10`, чтобы запрос не блокировал выполнение программы слишком долго при проблемах с сетью.
4. **Обработайте данные**:
* Извлеките данные из ответа в формате JSON с помощью метода `.json()`.
* Верните полученный результат (список праздников).
5. **Добавьте обработку исключений**:
* Оберните блок запроса в конструкцию `try / except`.
* В случае возникновения ошибки (например, сетевой сбой или неверный код страны) выведите описание ошибки в консоль через `print(e)`.
**Пример вызова функции:**
```python
from holiday_bot.holiday.handler import fetch_holidays_from_api

holidays = fetch_holidays_from_api(2025, "RU")
print(f"Загружено праздников: {len(holidays)}")

```
### Работа с git:
1. Склонируйте репозиторий задания к себе на компьютер.
2. Создайте свою новую ветку и переключитесь на неё: `git checkout -b 23_01_requests`.
3. Внесите изменения в файл `holiday_bot/holiday/handler.py`.
4. При необходимости продолжать работу в этом же репозитории, установите тег `ready_23`.

## Задание 10. Использование датаклассов для конфигурации

### Контекст:

По мере роста приложения количество настроек (URL-адреса, таймауты, пути к файлам, уровни логирования) увеличивается. Хранить их в виде разрозненных переменных или хардкодить в методах — плохая практика. Для структурирования таких данных в Python идеально подходят **dataclasses**.

### Цель задания:

Создать единую систему конфигурации бота, используя вложенные датаклассы, и интегрировать её в основной класс `MyHolidays`. Это позволит централизованно управлять поведением бота и упростит его тестирование.

### Что нужно сделать:

1. **Создайте модуль конфигурации**:
* В файле `holiday_bot/config_task.py` опишите четыре датакласса: `APIConfig`, `DatabaseConfig`, `LoggingConfig` и объединяющий их `AppConfig`.
* Рекомендуется использовать параметр `frozen=True`, чтобы настройки нельзя было случайно изменить во время работы программы.
* Реализуйте функцию `get_default_config()`, которая возвращает объект `AppConfig` со значениями по умолчанию.

2. **Интегрируйте конфиг в MyHolidays**:
* В файле `holiday_bot/custom_holiday_task.py` обновите конструктор `__init__`.
* Добавьте параметр `config`, который по умолчанию принимает значение `None`. Если конфиг не передан, функция должна вызвать `get_default_config()`.
* Замените "жестко прописанные" параметры (например, название файла JSON или кодировку) на значения, взятые из соответствующего раздела `self.config`.
* Внесите изменения в `config_task.py` и `custom_holiday.py`(Логирование и Источник данных) и `handler.py`(APIConfig) и `holiday_launch.py`(Логирование).

3. **Настройте логирование**:
* Используйте данные из `self.config.logging` для инициализации `logging.basicConfig` (уровень и формат сообщений).

4. **Внесите изменения в `handler.py` для url, timeout, retry**:

### Значения нужно брать из тех переменных, которые потом заменяете!

Заполнение - 
**APIConfig** - 
*     base_url - именно базовая ссылка без страны и года
*     timeout

**DatabaseConfig**:
*     file_path - только имя
*     encoding
...
```python
from config_task import AppConfig, APIConfig
from custom_holiday_task import MyHolidays

# Создаем свою настройку API
my_api_cfg = APIConfig(timeout=15)
# Собираем общий конфиг
custom_cfg = AppConfig(api=my_api_cfg)

# Передаем настройки в класс
bot_logic = MyHolidays(config=custom_cfg)
...
class MyHolidays(Russia):
    ...
    def __init__(self, config: AppConfig = None, **kwargs):
        ...
        self.config = config or get_default_config() # Вызов
        super().__init__(**kwargs)
        self.custom_holidays = self._load_custom_holidays()

    @log_exceptions
    def _load_custom_holidays(self) -> Dict[date, List[str]]:
        customholidays_file = Path(__file__).parent / self.config.db.file_path # Вызов

```

### Работа с git:

* Создайте новую ветку: `git checkout -b 24_01_dataclass`.
* При необходимости продолжать работу в этом же репозитории, установите тег `ready_24`.

**Что вы научитесь делать:**
* Организовывать сложные настройки через вложенные структуры данных.
* Разделять логику приложения и его конфигурацию.
* Работать с неизменяемыми (`frozen`) объектами для повышения стабильности кода.

## Задание 11. Асинхронные HTTP-запросы с aiohttp / asyncio

### Контекст:

Наш телеграм-бот работает на асинхронном движке `python-telegram-bot`, поэтому каждый метод, работающий с ботом, использует `async` для возможности параллельной обработки. Если использовать обычную библиотеку `requests`, бот будет "замирать" (перестанет отвечать другим пользователям) на все время, пока ждет ответа от API. Чтобы этого избежать, мы должны использовать неблокирующие запросы.

### Цель задания:

Реализовать асинхронную версию получения праздников, используя библиотеку `aiohttp`. Это позволит боту эффективно обрабатывать запросы от множества пользователей параллельно.

### Что нужно сделать:

В файле `holiday_bot/holiday/handler.py` реализуйте метод `fetch_holidays_from_api(year, country_code)`:

1. **Асинхронная сессия**: Используйте конструкцию `async with aiohttp.ClientSession() as session` для создания клиентской сессии.
2. **Асинхронный GET**: Выполните запрос к API Nager.Date, используя `await session.get(url, timeout=api.timeout)`.
3. **Обработка статуса**: Проверьте, что `response.status == 200`. Если статус другой — залогируйте ошибку и верните пустой список.
4. **Чтение данных**: Используйте `await response.json()` для получения данных из тела ответа. Обратите внимание, что в `aiohttp` чтение JSON — это тоже корутина (требует `await`).
5. **Безопасность**:
* Оберните код в `try/except`.
* Обработайте специфичные ошибки `aiohttp.ClientError` и `asyncio.TimeoutError` (и общий `Exception`).
* Верните пустой список `[]` в случае неудачи.

### Работа с git:

* Создайте новую ветку: `git checkout -b 25_01_asincio`.
* Реализуйте функцию и проверьте её работу.
* Закоммитьте изменения: `git commit -m `.
* Отправьте ветку в репозиторий: `git push`.
* Создайте Merge Request и при необходимости установите тег `ready_25`.
---
**Что вы изучите:**
* Разницу между синхронным и асинхронным сетевым взаимодействием.
* Работу с асинхронными контекстными менеджерами (`async with`).
* Правильную обработку таймаутов в асинхронной среде.
